...
function addNPCBones(npc:Entity, data:NPCData){
    switch(data.type){
        ...
        case NPCType.CUSTOM:
            ...
            idleAnim.set(npc, data && data.idleAnim ? data.idleAnim : 'Idle')
            Animator.playSingleAnimation(npc, idleAnim.get(npc))

            let npcData = npcDataComponent.get(npc)
            npcData.lastPlayedAnim = idleAnim.get(npc)

            if (data && data.walkingAnim) {
                walkingAnim.set(npc, data.walkingAnim)
...
function addClickReactions(npc:Entity, data:NPCData){
    let activateButton = data && data.onlyClickTrigger ? InputAction.IA_POINTER : InputAction.IA_POINTER
...
function walkNPC(npc:Entity, npcData:any, type:NPCPathType, duration:number, path:Vector3[], pointReachedCallback?:any, finishedCallback?:any){
    ...
    if (npcData.walkingAnim) {
        ...
        // }
        Animator.playSingleAnimation(npc, walkingAnim.get(npc), true)
        npcData.lastPlayedAnim = walkingAnim.get(npc)
      }
